...
public class CarManager {
    ...
    public boolean update(Car car) {
        if (this.getByID(car.getId()) == null) {
            Helper.showMsg(car.getId() + "updateError");
            return false;
    ...
    public ArrayList<Car> searchForBooking(String strt_date, String fnsh_date, Model.Type type, Model.Gear gear, Model.Fuel fuel){
        ...
        ArrayList<Book> bookedList = this.bookDao.selectByQuery(bookQuery);
        ArrayList<Integer> bookedCarId = new ArrayList<>();
        for (Book book : bookedList){
            bookedCarId.add(book.getCar_id());
        }

        searchedCarList.removeIf(car -> bookedCarId.contains(car.getId()));
